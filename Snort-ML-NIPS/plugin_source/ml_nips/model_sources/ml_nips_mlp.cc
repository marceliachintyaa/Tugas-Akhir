...
static constexpr float ML_NIPS_MATURE_THRESHOLD = 0.372f;    // MLP standalone threshold from thresholds_lgbm_mlp.json
static constexpr float ML_NIPS_EARLY_THRESHOLD = 0.372f;     // same MLP threshold for early/runtime scoring
...
static constexpr uint64_t ML_MIN_DURATION_US = 5000000ULL;
static constexpr uint64_t ML_MATURE_MIN_PACKETS = 10;
static constexpr uint64_t ML_MATURE_RECHECK_US = 1000000ULL;
...
static std::ofstream ids_log("/home/nadyahra/ml_ids_log_mlp.csv", std::ios::app);
static std::ofstream ips_log("/home/nadyahra/ml_ips_log_mlp.csv", std::ios::app);
...
MlNips::MlNips(uint16_t p, uint16_t m)
{
    ...
    // MLP-only model: 38 scaled flow features -> attack probability/score.
    mlp_model = tflite::FlatBufferModel::BuildFromFile("/opt/snort-ml/mlp_final.tflite");

    if (!mlp_model)
    {
        LogMessage("ml_nips: FAILED load MLP TFLite model: /opt/snort-ml/mlp_final.tflite\n");
        return;
    }
    ...
    if (!mlp_interpreter)
    {
        LogMessage("ml_nips: FAILED create MLP interpreter\n");
        return;
    }

    if (mlp_interpreter->AllocateTensors() != kTfLiteOk)
    {
        LogMessage("ml_nips: FAILED allocate tensors for MLP model\n");
        return;
    }

    LogMessage("ml_nips MLP tensor check: input_bytes=%zu output_bytes=%zu input_type=%d output_type=%d input_dims=%d output_dims=%d\n",
        mlp_interpreter->tensor(mlp_interpreter->inputs()[0])->bytes,
        mlp_interpreter->tensor(mlp_interpreter->outputs()[0])->bytes,
        mlp_interpreter->tensor(mlp_interpreter->inputs()[0])->type,
        mlp_interpreter->tensor(mlp_interpreter->outputs()[0])->type,
        mlp_interpreter->tensor(mlp_interpreter->inputs()[0])->dims->size,
        mlp_interpreter->tensor(mlp_interpreter->outputs()[0])->dims->size);

    LogMessage("ml_nips: MLP-only model loaded SUCCESS (38 scaled features -> score)\n");
...
*/
        ...
        if (mlp_interpreter)
        {
            // Feature scaling: same 38-feature order as AE-MLP script.
            ...
            /* CLEAN_LOG: noisy terminal debug disabled
            LogMessage("ml_nips MLP scaled input check: f0=%.6f f1=%.6f f15=%.6f f18=%.6f f33=%.6f\n",
                scaled_features[0], scaled_features[1], scaled_features[15],
                scaled_features[18], scaled_features[33]);
            */

            // MLP-only inference: 38 scaled features -> attack probability/score.
            float* mlp_input = mlp_interpreter->typed_input_tensor<float>(0);
            for (int i = 0; i < 38; i++)
                mlp_input[i] = scaled_features[i];
            ...
            LogMessage("ml_nips MLP prediction: score=%.8f threshold=%.2f stage=%s\n",
                score, active_threshold, is_mature_score ? "mature" : "early");

            // Pure ML decision logic.
            // Final decision uses only current MLP score and threshold.
            ...
            LogMessage("ml_nips decision: stage=%s score=%.8f threshold=%.2f ml_score_hit=%d detected=%d duration_us=%lu pkt_count=%lu syn_cnt=%lu rst_cnt=%lu\n",
            ...
            if (ml_score_hit)
            {
                LogMessage("ml_nips: ATTACK DETECTED BY MLP SCORE stage=%s score=%.8f threshold=%.2f\n",
                    stage_name, score, active_threshold);
        ...
        {
            LogMessage("ml_nips: MLP interpreter is NULL\n");
        }
    }
