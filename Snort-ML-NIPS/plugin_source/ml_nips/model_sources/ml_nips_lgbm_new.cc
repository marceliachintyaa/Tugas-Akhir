...
static constexpr float ML_NIPS_MATURE_THRESHOLD = 0.491f;    // LightGBM threshold from threshold_lgbm.pkl
static constexpr float ML_NIPS_EARLY_THRESHOLD = 0.491f;     // same LightGBM threshold for early/runtime scoring
...
// LightGBM ONNX Runtime objects.
// Model path  : /opt/snort-ml/models_new/lgbm_new.onnx
// Scaler path : /opt/snort-ml/models_new/scaler_params_lgbm_new.json
static Ort::Env lgbm_ort_env(ORT_LOGGING_LEVEL_WARNING, "ml_nips_lgbm");
static std::unique_ptr<Ort::Session> lgbm_session;

static std::vector<std::vector<float>> lgbm_quantiles;  // shape: n_quantiles x 38
static std::vector<float> lgbm_references;              // shape: n_quantiles
static float lgbm_threshold = ML_NIPS_MATURE_THRESHOLD;
static bool lgbm_ready = false;

// Forward declarations for LightGBM helper functions.
static bool load_lgbm_scaler(const char* path);
static bool load_lgbm_onnx_model();
static bool run_lgbm_pipeline(const std::vector<float>& raw_features, float& attack_prob);
// Dual CSV outputs:
...
static std::ofstream ids_log("/home/nadyahra/ml_ids_log_lgbm_new.csv", std::ios::app);
static std::ofstream ips_log("/home/nadyahra/ml_ips_log_lgbm_new.csv", std::ios::app);
...
MlNips::MlNips(uint16_t p, uint16_t m)
{
    ...
    const bool scaler_ok = load_lgbm_scaler("/opt/snort-ml/models_new/scaler_params_lgbm_new.json");
    const bool model_ok = load_lgbm_onnx_model();

    lgbm_ready = scaler_ok && model_ok;

    if (!lgbm_ready)
    {
        LogMessage("ml_nips: LightGBM pipeline NOT ready. Check /opt/snort-ml/models_new/lgbm_new.onnx and scaler_params_lgbm_new.json\n");
    }
    else
    {
        LogMessage("ml_nips: LightGBM pipeline loaded SUCCESS (QuantileTransformer + ONNX)\n");
    }
...
*/
        ...
        if (lgbm_ready)
        {
            ...
            if (!run_lgbm_pipeline(features, score))
            {
                LogMessage("ml_nips: LightGBM pipeline inference failed\n");
                return;
            }

            // The active threshold is 0.68 for LightGBM.
            // This value is also stored in scaler_params_lgbm_new.json for reference.

            LogMessage("ml_nips LGBM prediction: score=%.4f threshold=%.2f stage=%s\n",
...
*/
        ...
        {
            LogMessage("ml_nips: LightGBM pipeline is not ready\n");
        }
    }
...
//------------------------------------------------------------------------------
// LightGBM Scaler + ONNX Inference
//------------------------------------------------------------------------------
...
static bool load_lgbm_scaler(const char* path)
{
    try
    {
        ...
        if (!in.is_open())
        {
            LogMessage("ml_nips: FAILED open LightGBM scaler JSON: %s\n", path);
            return false;
        }
        ...
        lgbm_threshold = j.value("threshold", ML_NIPS_MATURE_THRESHOLD);
        lgbm_references = j.at("references").get<std::vector<float>>();
        lgbm_quantiles = j.at("quantiles").get<std::vector<std::vector<float>>>();

        if (lgbm_quantiles.empty() || lgbm_references.empty())
        {
            LogMessage("ml_nips: invalid LightGBM scaler JSON, empty quantiles/references\n");
            return false;
        }

        if (lgbm_quantiles[0].size() != 38)
        {
            LogMessage("ml_nips: invalid LightGBM scaler feature count: %zu, expected 38\n",
                lgbm_quantiles[0].size());
            return false;
        }

        if (lgbm_quantiles.size() != lgbm_references.size())
        {
            LogMessage("ml_nips: invalid LightGBM scaler shape: quantiles=%zu references=%zu\n",
                lgbm_quantiles.size(), lgbm_references.size());
            return false;
        }

        LogMessage("ml_nips: LightGBM scaler loaded SUCCESS threshold=%.4f quantiles=%zu features=38\n",
            lgbm_threshold, lgbm_quantiles.size());

        return true;
    }
    catch (const std::exception& e)
    {
        LogMessage("ml_nips: FAILED parse LightGBM scaler JSON: %s\n", e.what());
        return false;
    }
}

static float lgbm_quantile_scale(size_t feature_index, float value)
{
    if (feature_index >= 38 || lgbm_quantiles.empty() || lgbm_references.empty())
        return value;

    const size_t n = lgbm_references.size();

    const float first_q = lgbm_quantiles[0][feature_index];
    const float last_q  = lgbm_quantiles[n - 1][feature_index];

    if (value <= first_q)
        return lgbm_references[0];

    if (value >= last_q)
        return lgbm_references[n - 1];

    // Equivalent to sklearn QuantileTransformer interpolation:
    // find quantile interval then linearly interpolate into reference distribution.
    for (size_t i = 1; i < n; ++i)
    {
        const float q_left = lgbm_quantiles[i - 1][feature_index];
        const float q_right = lgbm_quantiles[i][feature_index];

        if (value <= q_right)
        {
            const float r_left = lgbm_references[i - 1];
            const float r_right = lgbm_references[i];

            if (std::fabs(q_right - q_left) < 1e-12f)
                return r_right;

            const float ratio = (value - q_left) / (q_right - q_left);
            return r_left + ratio * (r_right - r_left);
        }
    }
...
static bool load_lgbm_onnx_model()
{
    try
    {
        ...
        lgbm_session = std::make_unique<Ort::Session>(
            lgbm_ort_env,
            "/opt/snort-ml/models_new/lgbm_new.onnx",
            session_options
        );

        LogMessage("ml_nips: LightGBM ONNX model loaded SUCCESS\n");
        return true;
    }
    catch (const Ort::Exception& e)
    {
        LogMessage("ml_nips: FAILED load LightGBM ONNX model: %s\n", e.what());
        return false;
    }
}

static bool run_lgbm_onnx(const std::vector<float>& scaled_features, float& attack_prob)
{
    if (!lgbm_session || scaled_features.size() != 38)
        return false;

    try
    {
        std::array<int64_t, 2> input_shape = {1, 38};
        ...
        auto output_tensors = lgbm_session->Run(
        ...
        // probabilities shape is [1, 2] because ONNX was converted with zipmap=False.
        // class 0 = benign, class 1 = attack.
        attack_prob = probabilities[1];

        if (!std::isfinite(attack_prob))
            attack_prob = 0.0f;
    ...
    {
        LogMessage("ml_nips: LightGBM ONNX Invoke failed: %s\n", e.what());
        return false;
    }
}
...
static bool run_lgbm_pipeline(const std::vector<float>& raw_features, float& attack_prob)
{
    if (raw_features.size() != 38)
        return false;

    std::vector<float> scaled_features(38, 0.0f);

    for (size_t i = 0; i < raw_features.size(); ++i)
        scaled_features[i] = lgbm_quantile_scale(i, raw_features[i]);

    return run_lgbm_onnx(scaled_features, attack_prob);
}
