...
static constexpr float ML_NIPS_MATURE_THRESHOLD = 0.702f;    // model threshold from thresholds.json
static constexpr float ML_NIPS_EARLY_THRESHOLD = 0.702f;     // same ML threshold for early/runtime scoring
...
static std::ofstream ids_log("/home/nadyahra/ml_ids_log_aemlp_new.csv", std::ios::app);
static std::ofstream ips_log("/home/nadyahra/ml_ips_log_aemlp_new.csv", std::ios::app);
...
static inline bool ml_feature_ready(bool is_tcp, bool is_udp, bool is_icmp,
    uint64_t pkt_count, uint64_t bwd_pkt_count, uint64_t bwd_iat_sum,
    uint64_t syn_count)
{
    // UDP/ICMP has no TCP handshake/backward requirement. Wait for a minimal packet count
...
MlNips::MlNips(uint16_t p, uint16_t m)
{
    ...
    mlp_model = tflite::FlatBufferModel::BuildFromFile("/opt/snort-ml/models_new/ae_mlp_new.tflite");
...
void MlNips::eval(Packet* p)
{
    ...
    const bool is_icmp_packet = p->proto_bits & PROTO_BIT__ICMP;

    // NIPS scope: inspect TCP/UDP/ICMP flows.
    // ICMP is forced into the ML pipeline for experimental testing only.
    // Many TCP-specific features will remain zero for ICMP.
    if (!is_tcp_packet && !is_udp_packet && !is_icmp_packet)
        return;

    if (is_icmp_packet)
    {
        LogMessage("ml_nips: ICMP packet sent to ML pipeline src=%s dst=%s dsize=%u\n",
            src_ip, dst_ip, (unsigned)p->dsize);
    }
    ...
    if (is_icmp_packet)
    {
        // ICMP has no transport ports, so use a stable ICMP key to avoid flow splitting.
        client_ip = src;
        server_ip = dst;
        client_port = 0;
        server_port = 0;
    ...
    if (is_icmp_packet)
    {
        LogMessage("ml_nips: ICMP flow key=%s pkt_count_before=%lu client=%s server=%s cport=%u sport=%u icmp_force=1\n",
            key.c_str(), fs.pkt_count, client_ip.c_str(), server_ip.c_str(),
    ...
    else if (is_icmp_packet)
    {
        transport_header_len_for_len = 8; // ICMP header approximation
    }
...
*/
    ...
    const bool feature_ready_base = ml_feature_ready(
        is_tcp_packet, is_udp_packet, is_icmp_packet, fs.pkt_count, fs.bwd_pkts,
    ...
    const bool feature_ready = is_icmp_packet ? true : (feature_ready_base || spoofed_syn_ready);
