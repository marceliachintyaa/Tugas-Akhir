...
static constexpr float ML_NIPS_MATURE_THRESHOLD = 0.536f;    // LightGBM threshold from threshold_lgbm.pkl
static constexpr float ML_NIPS_EARLY_THRESHOLD = 0.536f;     // same LightGBM threshold for early/runtime scoring
...
// Model path  : /opt/snort-ml/lgbm_model_final.onnx
// Scaler path : /opt/snort-ml/scaler_params_lgbm.json
...
static std::ofstream ids_log("/home/nadyahra/ml_ids_log_lgbm.csv", std::ios::app);
static std::ofstream ips_log("/home/nadyahra/ml_ips_log_lgbm.csv", std::ios::app);
...
MlNips::MlNips(uint16_t p, uint16_t m)
{
    ...
    const bool scaler_ok = load_lgbm_scaler("/opt/snort-ml/scaler_params_lgbm.json");
    ...
    if (!lgbm_ready)
    {
        LogMessage("ml_nips: LightGBM pipeline NOT ready. Check /opt/snort-ml/lgbm_model_final.onnx and scaler_params_lgbm.json\n");
    }
    else
    {
...
*/
        ...
        if (lgbm_ready)
        {
            ...
            // This value is also stored in scaler_params_lgbm.json for reference.
...
static bool load_lgbm_onnx_model()
{
    try
    {
        ...
        lgbm_session = std::make_unique<Ort::Session>(
            lgbm_ort_env,
            "/opt/snort-ml/lgbm_model_final.onnx",
            session_options
        );
