...
static constexpr float ML_NIPS_MATURE_THRESHOLD = 0.48f;    // MLP standalone threshold from thresholds_lgbm_mlp.json
static constexpr float ML_NIPS_EARLY_THRESHOLD = 0.48f;     // same MLP threshold for early/runtime scoring
...
static std::ofstream ids_log("/home/nadyahra/ml_ids_log_mlp_new.csv", std::ios::app);
static std::ofstream ips_log("/home/nadyahra/ml_ips_log_mlp_new.csv", std::ios::app);
...
MlNips::MlNips(uint16_t p, uint16_t m)
{
    ...
    mlp_model = tflite::FlatBufferModel::BuildFromFile("/opt/snort-ml/models_new/mlp_new.tflite");

    if (!mlp_model)
    {
        LogMessage("ml_nips: FAILED load MLP TFLite model: /opt/snort-ml/models_new/mlp_new.tflite\n");
        return;
    }
